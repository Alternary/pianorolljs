...
async function playChordyPartitionedSampleSong3() {
  ...
  let bassOn = true
  let drillOn = true
  let seed = '32'//'27'//'13'//'10'//'8'//'5'//'4'//'l'//'lol'
  ...
  let drillingPattern = musicUtils.randomFloatContinuum(0.03, 999, 16, seed)
...
async function playMoreChordySong() {
  ...
  let seed = '36'//'27'//'13'//'10'//'8'//'5'//'4'//'l'//'lol'
...
async function playReharmonicSong() {
  let drumOn = false
  let chordOn = true//with chords on, the drum timing wonks up, unless very few melodies are played at a time
  let sampleOn = false//true
  ...
  melodies = utils.groupBy(musicUtils.generateOverAndUnderToneSequence(seed, 9, 5, 2, musicUtils.randomFloatContinuum(0.1, 99 * (melodyAmount + 1), 32, seed)).map(freq => musicUtils.freqToInt(freq)), melodyAmount + 1); melodies = utils.zipLists(melodies) //need to transpose as they get retransposed down below
...
async function playMultiharmonicSong() {
  let drumOn = false
  let chordOn = true//with chords on, the drum timing wonks up, unless very few melodies are played at a time
  let sampleOn = false//true
  let bassOn = true//false//true
  ...
  melodies = utils.groupBy(musicUtils.generateOverAndUnderToneSequence(seed, 9, 5, 2, musicUtils.randomFloatContinuum(0.1, 99 * (melodyAmount + 1), 32, seed)).map(freq => musicUtils.freqToInt(freq)), melodyAmount + 1); melodies = utils.zipLists(melodies) //need to transpose as they get retransposed down below
  melodies = musicUtils.generateMultiHarmonicSequences(melodyAmount + 1, 0.1, 999, 9, 5, 2, seed).map(sequence => sequence.map(freq => musicUtils.freqToInt(freq)))
  let bassline = melodies[melodyAmount]
  melodies = melodies.slice(0, melodyAmount) //leaving last melody as bassline
  console.log('melodies[0]', melodies[0])
  let melodyColumns = utils.zipLists(melodies)
